...
if __name__ == '__main__':
    ...
    # 获取相关参数,BUG:相对路径报错：os.walk
    source_img_json_path = 'E:\My Projects\mask rcnn_test\dataset\dataprocess_rock_dataset'  # 图片和json文件原始位置
    save_img_json_path = 'E:\My Projects\mask rcnn_test\dataset\enhanced_dataprocess_rock_dataset'  # 图片增强结果保存文件
    ...
    for parent, _, files in os.walk(source_img_json_path):
        ...
        for file in files:
            if file.endswith('jpg') or file.endswith('png') or file.endswith('BMP'):
                ...
                # 保存源文件
                shutil.copyfile(pic_path,os.path.join(save_img_json_path,file))
                shutil.copyfile(json_path,os.path.join(save_img_json_path,file[:-4])+'.json')
